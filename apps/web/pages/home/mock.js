...
export default function Home() {
  ...
  const handleGroupClick = (group) => {
    ...
    router.push(`/groupchat/mock?roomId=${group.id}&username=${encodeURIComponent(username)}`);
  };
  ...
  const handleSearchUsers = async (searchText) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/profile/search?${searchParam}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const handleStartDM = async (userId) => {
    ...
    try {
      ...
      const username = user?.username || 'User';
      router.push(`/groupchat/mock?roomId=${dm.id}&username=${encodeURIComponent(username)}`);
    } catch (err) {
        ...
        <div style={{ 
          ...
          <div>
            <div style={{
              ...
              <SearchUserModal
                show={showStartDM}
                title={<><span>💌</span> <span>Start Direct Message</span></>}
              ...
              <SearchUserModal
                show={showAddMember}
                title={<><span>👥</span> <span>Add Member to "{selectedGroup?.name}"</span></>}
                searchValue={searchUsername}
                searchResults={searchResults}
                onSearchChange={(value) => {
                  setSearchUsername(value);
